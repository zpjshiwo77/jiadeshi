...
function importCom() {
	...
	com.init = function(callback) {
		...
		function html_resize() {
			...
			if(dir == 'portrait') {
				var iphoneXFull=os.iphoneX && window.innerHeight>=700;
				...
				var iphoneXFull=os.iphoneX && window.innerWidth>=700;
				if(ibase.landscapeScaleMode == 'cover' || ibase.landscapeScaleMode == 'contain') {
	...
	com.screenScrollEnable = function() {
		...
		if(ibase.dir=='portrait'){
			...
			if(os.ios) article.css({'position': 'relative'});
	...
	com.screenScrollUnable = function() {
		...
		if(ibase.dir=='portrait'){
			...
			if(os.ios) article.css({'position': 'fixed'});
